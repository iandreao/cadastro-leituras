...
export const SUPER_ADMIN_EMAIL = "sistemacond@gmail.com";
...
export async function garantirSchemaMultiTenant(db: ClienteSql = getPrisma()) {
  ...
  await executarSePossivel(
    db,
    `ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "primeiroAcesso" BOOLEAN NOT NULL DEFAULT true`,
  );
  await executarSePossivel(
    db,
  ...
  await executarSePossivel(
    db,
    `ALTER TABLE "Condominio" ADD COLUMN IF NOT EXISTS "versao" INTEGER NOT NULL DEFAULT 1`,
  );
  await executarSePossivel(
    db,
